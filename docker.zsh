...
function Initialize-Docker-Variables {
    echo 'Initializing Docker variables'
    echo -n 'Enter image name to create or use [linux-arm(default)]: '; read IMAGE
    echo -n 'Enter container name [linux-arm(default)]: '; read CONTAINER
    echo -n 'Enter network name [linux-arm(default)]: '; read NETWORK
    echo -n 'Enter docker username [himanshu(default)]: '; read DOCKER_USER
    echo -n 'Enter base image name to (if) create above image from [osrf/ubuntu_armhf:focal(default)]: '; read BASE_IMAGE
    echo -n 'Enter shell to be used in container [zsh(default)]: '; read DOCKER_SHELL
    echo -n 'Save container image? [true(default)/false]: '; read DOCKER_SAVE
    export IMAGE=${IMAGE:-'linux-arm'}
    export CONTAINER=${CONTAINER:-'linux-arm'}
    export NETWORK=${NETWORK:-'linux-arm'}
    export DOCKER_USER=${DOCKER_USER:-'himanshu'}
    ...
    export DOCKER_SHELL=${DOCKER_SHELL:-'zsh'}
    export DOCKER_SAVE=${DOCKER_SAVE:-true}
    echo 'Docker variables initialized'
}
